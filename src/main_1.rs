...
// ==========================================
// Theme & Palette (Catppuccin Mocha Style)
// ==========================================
...
impl Theme {
    pub fn mocha() -> Self {
        Self {
            border: parse_hex_color("#cba6f7"),       // Lavender
            title: parse_hex_color("#f5c2e7"),        // Pink
            active: parse_hex_color("#a6e3a1"),       // Green
            inactive: parse_hex_color("#f38ba8"),     // Red
            text: parse_hex_color("#cdd6f4"),         // Text
            highlight_bg: parse_hex_color("#313244"), // Surface1
            highlight_fg: parse_hex_color("#f9e2af"), // Yellow
            accent: parse_hex_color("#89b4fa"),       // Blue
            default_tag: parse_hex_color("#fab387"),   // Peach
            search: parse_hex_color("#74c7ec"),       // Sapphire
            status: parse_hex_color("#a6adc8"),       // Subtext0
        }
    }
}
...
fn handle_key_input(app: &mut App, key: KeyEvent) {
    ...
    match key.code {
        ...
        KeyCode::Char('r') => {
            app.status_msg = "Refreshing status...".to_string();
...
fn draw(f: &mut Frame, app: &mut App) {
    let chunks = Layout::default()
        ...
        .constraints([
            ...
            Constraint::Length(3), // Tuiplay-style Shortcut Footer
        ])
        .split(f.area());
...
fn render_help_tab(f: &mut Frame, area: Rect, app: &App) {
    ...
    let content = vec![
        ...
        Line::from(vec![Span::styled("  r               ", key_style), Span::styled("Refresh NetworkManager statuses & scan Downloads", desc_style)]),
...
fn draw_help_popup(f: &mut Frame, app: &App) {
    ...
    let content = vec![
        ...
        Line::from(vec![Span::styled("  r              ", key_style), Span::styled("Refresh active status & network list", desc_style)]),
